    ...
    public static class EngineRenderer
    {
        ...
        public static void Texture(Texture2D texture, RectShape source, RectShape dest, int rotation)
        {
            Raylib.DrawTexturePro(
                texture, source.toRaylib(), dest.toRaylib(), Coordinates.Zero().ToVector(), rotation, Color.White
            );
        }
        ...
        public static void TextOrigined(float x, float y, Coordinates origin, string text, Color color, int fontSize = 24)
        {
            if (origin.IsZero())
            {
        ...
        public static void Line(Coordinates start, Coordinates end, Raylib_cs.Color color)
        {
